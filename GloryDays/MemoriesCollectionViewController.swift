...
//  GloryDays
//
//  Created by Cristhian Parrales on 14/5/17.
...
class MemoriesCollectionViewController: UICollectionViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate, AVAudioRecorderDelegate, UISearchBarDelegate {
    ...
    */
    ...
    func finishRecordingMemory(success: Bool) {
        ...
        collectionView?.backgroundColor = UIColor(red: 97.0/255, green: 86.0/255, blue: 110.0/255, alpha: 1.0)
        audioRecorder?.stop()
    ...
    func transcribeAudioToText(memory: URL) {
        ...
        let transcrition = transcriptionURL(for: memory)
        ...
        recognizer?.recognitionTask(with: request, resultHandler: { [unowned self] (result, error) in
            ...
            if result.isFinal {
                ...
                do {
                    try text.write(to: transcrition, atomically: true, encoding: String.Encoding.utf8)
                    self.indexMemory(memory: memory, text: text)
                } catch {
                    print("Ha habido un error al tratar de guardar la traduccion")
                }
            }
        })
    }
    ...
    func indexMemory(memory: URL, text: String){
        ...
        let item = CSSearchableItem(uniqueIdentifier: memory.path, domainIdentifier: "com.cristhian", attributeSet: attributeSet)
    ...
    override func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        do {
            ...
            let transcritionName = transcriptionURL(for: memory)
            ...
            if fileManager.fileExists(atPath: transcritionName.path) {
                let contents = try String(contentsOf: transcritionName)
                print(contents)
            }
        } catch {
